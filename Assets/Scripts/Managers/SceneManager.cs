...
public class SceneManager : MonoBehaviour {
        ...
        get {
            lock (lockObject){
                if ( _instance == null ){
                    ...
                    if ( _instance == null ){
                        GameObject o = new GameObject("SceneManager");
                        _instance = o.GetComponent<SceneManager>();
                    }
                }
            }
            return _instance;
    ...
    public static void LoadScene(string s){
        ...
        if ( s.ToLower().Contains("tutorial") || s.ToLower().Contains("petselect") ){
            ...
            foreach (SceneObject so in instance.playerScene.sceneObjects){
                ...
                if ( o.layer == LayerMask.NameToLayer("Ground") ){
                    BoxCollider bc = o.GetComponent<BoxCollider>();
                    if ( bc.bounds.min.x < minX )
                        minX = bc.bounds.min.x;
                    if ( bc.bounds.max.x > maxX )
                        maxX = bc.bounds.max.x;
                }
            }
            instance.currentScene = instance.playerScene;

            CameraControls.instance.SetRect(new Rect(minX,-1f,maxX-minX,25f));
        } else if ( s.ToLower().Contains("combat") ){
            
        } else {
            GameObject o = (GameObject) Instantiate(Resources.Load("SceneObjects/DungeonGenerator"));
            o.GetComponent<DungeonGenerator>().Initialize(s);
            instance.currentScene = new Scene();
            instance.currentScene.name = s;
            instance.currentScene.gameObjects.Add(o);
        }
    }
}
